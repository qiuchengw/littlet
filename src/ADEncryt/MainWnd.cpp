...
BOOL MainWnd::ZipData(__in const CStdString&sDataPath,__out QBuffer &bufData)
{
	// 扫描文件
	QBuffer bufFile;
	...
	// 创建压缩文件
	CXZip zip;
	CStdString sTempZip = sDataPath + L"/__temp_file.zip";
...
{
	CStdString sPicPath = EFilePath(GetCtrl("#path_picture")).GetFilePath();
	CStdString sDataPath = EFolderPath(GetCtrl("#path_data")).GetFolderPath();
...
{
	CStdString sPicPath = EFilePath(GetCtrl("#path_picture")).GetFilePath();
	CStdString sSavePath = EFolderPath(GetCtrl("#path_data")).GetFolderPath();
	...
	CStdString sFileName = L"/__Data__.dat";
