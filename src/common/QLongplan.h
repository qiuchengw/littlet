...
public:
	...
	inline CStdString Item()const { return m_sItem; }
	inline CStdString Notes()const { return m_sNotes; }
	inline CStdString Attachs()const { return m_sAttachs; }
...
public:
	...
	inline CStdString Goal()const { return m_sGoal; }
...
public:
	...
	inline int ID()const { return m_nID; }
	inline CStdString Des()const { return m_sDes; }
	inline CStdString Stage()const { return m_sStage; }
	...
	// ret: 0年3月2天
	CStdString GetTimeFlee();  // 从计划开始到现在已经过去了多久
...
public:
	...
	LONG	m_nFlag;
	CStdString	m_sStage;		// 阶段计划描述 如：吉他-》基础练习
	CStdString m_sDes;		// 附加描述，
...
struct TAchievement 
{
	int			nID;
	int			nParentID;	// 所属
	CStdString		sAchievement;	// 文本描述		
	CStdString		sAttachment;	// 附件ID列表； 形如 "2;4;35;"		
};
