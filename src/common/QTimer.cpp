...
void QTimer::ResetAllFiled()
{
	...
	m_nID = INVALID_ID;
	eflag_exec_ = AUTOTASK_EXEC_NOTSET;
...
BOOL QTimer::ParseAbsoluteExp( const CStdString& sExp )
{
	if (sExp.IsEmpty())
		return FALSE;
	CStdString sExpTest = sExp, sValue;
	WCHAR cProp;
		...
		case L'A':
			{
				...
				eflag_exec_ = (ENUM_AUTOTASK_EXECFLAG)StrToInt(sValue);
				break;
			}
		case L'X':
			{
...
BOOL QTimer::ParseRemindExp( const CStdString&sExp,__out int &nA, 
    __out WCHAR&cAUnit,__out CStdString&sSound,__out CStdString&sMsg )
{
    WCHAR cProp;
    CStdString sPart,sValue,sTemp = sExp;
    int idx;
    while (true)
    {
        idx = sTemp.Find(L";\n");
...
BOOL QTimer::ParseRelateExp( const CStdString& sExp )
{
	if (sExp.IsEmpty())
		return FALSE;
	CStdString sExpTest = sExp;
	WCHAR cProp;
	CStdString sValue;
	while (!sExpTest.IsEmpty())
	{
...
BOOL QTimer::_Parse( __inout CStdString&sExp ,
	__out WCHAR& cProp, __out CStdString& sValue )
{
	int idx = sExp.Find(L'=');
...
BOOL QTimer::_ParseSpanTime( __in const CStdString &sExp , 
...
BOOL QTimer::SetExp( const CStdString& sExp )
{
	if (sExp.IsEmpty())
	{
		ASSERT(FALSE);
...
// 			m_tmLifeEnd.Format(L"%Y/%m/%d %H:%M"));
		// after
		when_des.Format(L"在<b .yellow>[%s] [%d][%s]</b>之后",
		...
		if (IsExecSpan2())
		{
			time_part.Format(L"然后每 <b .yellow>[%d][%s]</b>执行",
				GetExecSpan2(),
			...
			if (IsExecCount())
			{
				time_part.Format(L"在 <b .yellow>[%d]</b> 次后停止",
					GetExecCount());
				when_des += L"<br />" + time_part;
			}
		}
	...
	else
	{
        ...
        for (int i = 0; i < m_arTime.size(); ++i)
        {
            tm_test = QTime::ParseTime(m_arTime[i]);
            time_part.AppendFormat(tm_test.Format(L"%H:%M:%S,"));
        }
        ...
        case AUTOTASK_EXEC_ATYEARDAY:
            {
                ...
                if (0 != m_dwSpan)
                {
                    tmp.Format(L" - 提前%d天", m_dwSpan);
                }
                CStdString date_part;
                date_part.Format(L"%d/%d%s", tm_test.GetMonth(), tm_test.GetDay(), tmp);
                when_des.Format(L"在 <b .yellow>[%s] [%s]</b>", date_part, time_part);
                break;
            }
		case AUTOTASK_EXEC_ATDATE:
			{
				tm_test = QTime::ParseDate(m_arX[0]);
				when_des.Format(L"在 <b .yellow>[%s] [%s]</b>",
		...
		case AUTOTASK_EXEC_ATDAILY:
			{
				ASSERT(m_arX.size() == 1);
				when_des.Format(L"每 <b .yellow>[%d]</b> 天的 <b .yellow>[%s]</b>",
                    m_arX[0],time_part);
				break;
			}
		case AUTOTASK_EXEC_ATWEEKDAY:
			{ // 星期x执行
				...
				when_des.Format(L"<b .yellow>星期[%s]</b>(0-周日,...,6-周六)的 [%s]<br />",sWeekdays,time_part);
				break;
			}
		case AUTOTASK_EXEC_ATMONTHDAY:
			{
				...
				when_des.Format(L"每 <b .yellow>月[%s]</b> 的 [%s]",days,time_part);
				break;
			}
        default:
            return FALSE;
