...
CStdString QHelper::UniqueString()
{
	CStdString str;
...
CStdString QHelper::GetTimeUnitString(WCHAR cUnit)
{
	switch (cUnit)
	{
...
BOOL QHelper::MakeAbsExp(ENUM_AUTOTASK_EXECFLAG flag,QTime& tmB, QTime& tmE,
    ...
    __out CStdString& sResultExp, __out CStdString& sError)
{
    ...
    { // 像生日一样，有提前量
        ...
        {
            sError = L"按年执行     参数只能有2个（[0]:执行日期，[1]:提前量）";
            return FALSE;
        }

        sResultExp.Format(L"A=%d;Q=%d;X=%s;T=%s;",AUTOTASK_EXEC_ATYEARDAY,
            StrToInt(arDatePots[1]), arDatePots[0], sTimePots);
        return TRUE;
    }
...
CStdString QHelper::MakeRemindExp( int nA, WCHAR cAUnit, 
    const CStdString&sSound,const CStdString&sMsg )
{
