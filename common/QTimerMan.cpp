...
{
	TimerListItr itrEnd = _EndItr(m_lstTimer);
	for (TimerListItr itr = _BeginItr(m_lstTimer);
		itr != itrEnd; ++itr)
	{
		if ((*itr)->ID() == nID)
			return itr;
...
{
	for (TimerListItr i = _BeginItr(m_lstTimer); i != _EndItr(m_lstTimer); ++i)
	{
