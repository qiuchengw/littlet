...
绝对时间表达式：
	...
	S,E: Start Time/ End Time , 
		比如在一天中的 [13:20:20]~[22:30:30]
	P: 间隔日期，单个INT值，单位天，A=TASK_EXEC_ATDAILY有效
		比如每隔3天 P=3;
...
public:
    ...
    static BOOL ParseRemindExp( const QString&sExp,__out int &nA,
        __out WCHAR&cAUnit,__out QString&sSound,__out QString&sMsg );

    BOOL GetRemindString(__out QString& sReminderDes);
    //----------------------------------------------------------
	// status
	// 定时器是否已经启动
	BOOL IsStarted()const { return m_hTimerTask != NULL; }
	...
	QTime NextExecTime()const { ASSERT(IsStarted()); return m_tmNextExec; }
	// 检查任务是否合法或过期，并不真正设定定时器
	ENUM_AUTOTASK_RUNNING_STATUS TestStart( );
	BOOL IsReminderEnabled()const { return !(m_dwTimerFlag & TIMER_FLAG_REMINDERDISABLED); }
	BOOL EnableReminder( HANDLE hTimerQueue,int nTaskID,BOOL bEnable=TRUE );

	//----------------------------------------------------------
	BOOL IsTheYear(int y)const;
	int ID()const { return m_nID; }
	inline BOOL IsValid()const { return m_nID != INVALID_ID; }
	inline BOOL IsRelateTimer() { return m_eTimerType == TIMER_TYPE_RELATE; }
	inline ENUM_AUTOTASK_EXECFLAG GetExecFlag()const { return m_eExecFlag; }
	...
	QTime GetLifeBegin() { return m_tmLifeBegin; }
	QTime GetLifeEnd() { return m_tmLifeEnd; }

	BOOL SetExp(const QString& sExp);

	int GetTimerID()const { return m_nID; }
...
public: // relate
	DWORD GetExecSpan()const { return m_dwSpan; }
	// 解析为秒返回
	DWORD GetExecSpanSeconds()const ;
	WCHAR GetExecSpanUnit()const { return m_cSpanUnit; }
	DWORD GetExecSpan2()const { return m_dwSpan2; }
	DWORD GetExecSpanSeconds2()const ;
	WCHAR GetExecSpanUnit2()const { return m_cSpanUnit2; }
	DWORD GetExecCount()const { return m_iExecCount; }
	// 执行第一次后是否再间隔执行
	BOOL IsExecSpan2()const { return (m_dwSpan2>0);}
	BOOL IsExecCount()const { return m_iExecCount > 0; }
...
public: // abs 
	...
	**/
    ...
    DWORD GetExecDate( );
	QString GetXFiled()const { return m_sXFiledExp; }
	QString GetRemindExp()const { return m_sExpRemind; }
	QString GetWhenExp()const { return m_sExpWhen; }
    BOOL GetWhenDoString(QString &sWhenDoDes);

protected:
	HANDLE GetTimerHandle() { return m_hTimerTask; }
...
protected:
	...
	BOOL ParseExp(const QString& sExp);
	BOOL ParseRelateExp(const QString& sExp);
	BOOL ParseAbsoluteExp(const QString& sExp);
	...
	//	sProp = r;
	//	sValue = 1;
	static BOOL _Parse(__inout QString&sExp ,__out WCHAR& sProp, __out QString& sValue);
	// 200[s|m|h]
	// 200m = 200 * 60s;  cUnit = m;
	// 200h = 200 * 3600s	cUnit = h;
	// 200s = 200s;		cUnit = s;
	BOOL _ParseSpanTime(__in const QString &sExp , __out WCHAR& cUnit, __out DWORD& dwSpan );
	
	// 2,4,8,9 
	// 以 英文‘,’ 分隔的数字字符，转换到整数值
	BOOL _ParseToIntArray( __inout QString& sExp,__out IntArray & ar );
...
private:
	int				m_nID;	// 数据库中的id
	QString			m_sExpWhen;		// 何时做
	QString			m_sExpRemind;	// 何时提醒
	ENUM_AUTOTASK_EXECFLAG	m_eExecFlag;	
	ENUM_TIMER_TYPE		m_eTimerType;		// 相对时间还是绝对时间任务
	QTime			m_tmLifeBegin;	// 任务的生效时间
	QTime			m_tmLifeEnd;	// 任务过期时间
	QString			m_sXFiledExp;	// 
