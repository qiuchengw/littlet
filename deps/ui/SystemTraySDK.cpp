    ...
    UINT uBalloonTimeout /*=10*/)
{
    ...
    m_nMaxTooltipLength = ARRAYDIM(m_tnd.szTip);
    ...
    wcsncpy_s(m_tnd.szTip, ARRAYDIM(m_tnd.szTip), szToolTip, m_nMaxTooltipLength);
    ...
    if (m_bWin2K && szBalloonTip)
    {
        ...
        wcsncpy_s(m_tnd.szInfo, ARRAYDIM(m_tnd.szInfo), szBalloonTip, 255);
        if (szBalloonTitle)
            wcsncpy_s(m_tnd.szInfoTitle, ARRAYDIM(m_tnd.szInfoTitle), szBalloonTitle, 63);
        else
...
BOOL CSystemTray::SetTooltipText(LPCTSTR pszTip)
{
    ...
    wcsncpy_s(m_tnd.szTip, ARRAYDIM(m_tnd.szTip), pszTip, m_nMaxTooltipLength-1);
...
LPTSTR CSystemTray::GetTooltipText() const
{
    ...
    wcsncpy_s(strBuffer, ARRAYDIM(strBuffer), m_tnd.szTip, m_nMaxTooltipLength-1);
    ...
    UINT    uTimeout /*=10*/ )
{
    ...
    m_tnd.uFlags = NIF_INFO;
    wcsncpy_s(m_tnd.szInfo, ARRAYDIM(m_tnd.szInfo), szText, 256);
    if (szTitle)
        wcsncpy_s(m_tnd.szInfoTitle, ARRAYDIM(m_tnd.szInfoTitle), szTitle, 64);
    else
        m_tnd.szInfoTitle[0] = _T('\0');
...
void CSystemTray::MinimizeToTray(HWND hWnd)
{
#ifndef _WIN32_WCE
