...
public:
	...
	inline bool SetReferrer(const wstring &sRef);
    ...
    if (::WinHttpCrackUrl(m_requestURL.c_str(), m_requestURL.size(), 0, &urlComp))
    {
        ...
        if (hConnect != NULL)
        {
            ...
            hRequest = ::WinHttpOpenRequest(hConnect,
                                            ...
                                            urlComp.lpszUrlPath,
                                            NULL,
                                            /*WINHTTP_NO_REFERER*/m_sRefferer.c_str(),
            ...
            if (hRequest != NULL)
            {
                ...
                while (!bGetReponseSucceed && iRetryTimes++ < INT_RETRYTIMES)
                {
                    ...
                    {
                        if (!::WinHttpAddRequestHeaders(hRequest, m_additionalRequestHeaders.c_str(),
							m_additionalRequestHeaders.size(), WINHTTP_ADDREQ_FLAG_COALESCE_WITH_SEMICOLON))
                        {
                            m_dwLastError = ::GetLastError();
                        }
                    }
                    if (m_additionalRequestCookies.size() > 0)
                    {
                        ...
                        if (!::WinHttpAddRequestHeaders(hRequest, cookies.c_str(), cookies.size(), 
...
bool WinHttpClient::SetReferrer( const wstring &sRef )
{
	m_sRefferer = sRef;
