...
{
	m_bEditMode = FALSE;
	m_tmLifeBegin = m_tmLifeEnd = QTime::GetCurrentTime();
}
...
LRESULT QExecTimeDlg::OnDocumentComplete()
{
	...
	// 生命期
	EDate(GetCtrl("#date_begin")).SetDate(m_tmLifeBegin);
	EDate(GetCtrl("#date_end")).SetDate(m_tmLifeEnd);
	ETime(GetCtrl("#time_begin")).SetTime(m_tmLifeBegin);
	ETime(GetCtrl("#time_end")).SetTime(m_tmLifeEnd);
...
BOOL QExecTimeDlg::GetRelateExp(__out QString & sExp)
{
...
BOOL QExecTimeDlg::GetAbsoluteExp( __out QString &sExp ,QTime&tmB,QTime &tmE)
{
	GetLifeTime(tmB,tmE);
...
//	if (ERadioBox(GetCtrl("#rio_exectype_relate")).IsChecked())
	{// relate
		GetLifeTime(m_tmLifeBegin,m_tmLifeEnd);
		if (!GetRelateExp(m_sExp))
			return FALSE;
	}
	else	// ABS
	{
		if (!GetAbsoluteExp(m_sExp,m_tmLifeBegin,m_tmLifeEnd))
			return FALSE;
	}
	QTimer testTimer;
	if (!testTimer.SetLifeTime(m_tmLifeBegin,m_tmLifeEnd))
	{
...
void LAddEventDlg::OnClkExecTime( HELEMENT )
{
	...
	{
		m_tmLifeBegin = pDlg->m_tmLifeBegin;
		m_tmLifeEnd = pDlg->m_tmLifeEnd;
...
{
    ASSERT(NULL != pTimer);
    QString str = L"<b .yellow>[" + pTimer->GetLifeBegin().Format(L"%c") 
...
BOOL LAddEventDlg::GetRemindExp(QString &sRmdExp)
{
    QString sUnit,sSound,sMsg;
...
void LAddEventDlg::SetDoWhat( ENUM_AUTOTASK_DOWHAT eDo ,const QString& sTask )
{
...
void LAddEventDlg::SetRemindExp( const QString &sRmdExp )
{
